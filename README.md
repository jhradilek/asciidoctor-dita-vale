...
| MapAttribute | AsciiDoc assemblies support the `:chunk-to-content:` and `:navtitle:` attributes as a way to assign the `chunk="to-content"` and `navtitle` attributes to the DITA concept they convert to. Do not use `:chunk-to-content:` or `:navtitle:` attribute definitions in AsciiDoc maps. |
| MapChunk | AsciiDoc assemblies and maps use the `chunk="to-content"` attribute to make the topic and any topics nested under it part of a single page. Do not add `chunk="to-content"` to any topics that are nested under a topic that already has this attribute assigned. |
...
| MapFilename | [DITA-OT](https://www.dita-ot.org/) and various component content management systems impose additional restrictions on what characters can appear in DITA file names. To prevent problems, only use lowercase alphanumeric characters and hyphens (`a-z`, `0-9`, and `-`) in your file names. |
| MapLevel | AsciiDoc assemblies and maps use the [leveloffset attribute value](https://docs.asciidoctor.org/asciidoc/latest/directives/include-with-leveloffset/) to define the intended topic hierarchy. Make sure that the `leveloffset` values increment by one and do not use a value that is lower than in the first included topic. |
| MapSize | To ensure optimum performance in component content management systems (CCMS), do not use more than 200 include directives in AsciiDoc assemblies and maps. |
| MapToc | AsciiDoc assemblies and maps use the `chunk="to-content"` attribute to make the topic and any topics nested under it part of a single page. Add `toc="no"` to any topics that are nested under a topic with the `chunk="to-content"` attribute. |
...
| TaskContents | To correctly map contents to `<steps>` or `<steps-unordered>` in DITA 1.3, all procedure modules must contain the *Procedure* [block title](https://docs.asciidoctor.org/asciidoc/latest/blocks/add-title/) in AsciiDoc. |
...
| AttributeDefinition | Lists all [attribute definitions](https://docs.asciidoctor.org/asciidoc/latest/attributes/built-in-attributes/) in the file that potentially influence the DITA output.  Use this information to anticipate potential problems with the conversion. |
...
| IgnoredFile | Lists all content type definitions with the value "IGNORE". Use this information to list files that are not intended for conversion. |
...
| IntrinsicAttribute | Lists all [intrinsic attribute references](https://docs.asciidoctor.org/asciidoc/latest/attributes/document-attributes-ref/#intrinsic-attributes) in the file. Use this information to anticipate potential problems with the conversion. |
...
| SnippetLink | Lists all links and cross references in snippet files. Use this information to identify snippets that might bring links and cross references to the main body of a concept topic during conversion. |
...
| MAP | Identifies the AsciiDoc file as an AsciiDoc map intended to convert directly to a DITA map. This implies that the file only contains include directives, an optional document title, an optional ID, and no other printable content. |
